...
const TeacherPage = () => {
  ...
  const fetchStudents = () => {
    fetch(`${apiUrl}/view/students`) // Adjust API endpoint if necessary
      ...
      .then(data => {
        ...
        const filteredStudents = data.students.filter(student => student.teacher && student.teacher._id === userId);
  ...
  return (
    <div className="container-ss container mx-auto p-4">
      ...
      <AddStudent isOpen={showAddStudent} onClose={() => { toggleAddStudent(); fetchStudents(); }} teacherId={userId}/>
      <h1 className="text-3xl font-bold mb-6 text-center">Class Students</h1>
